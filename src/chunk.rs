...
impl Chunk {
    ...
    pub fn data_as_string(&self) -> Result<String> {
        String::from_utf8(self.data.clone()).map_err(|_| "Invalid UTF-8".into())
    }
...
impl TryFrom<&[u8]> for Chunk {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        let crc =
            u32::from_be_bytes(bytes[(8 + length as usize)..(12 + length as usize)].try_into()?);
        //  Check the CRC
        let crc_calculated: Crc<u32> = Crc::<u32>::new(&CRC_32_ISO_HDLC);
        let crc_calculated = crc_calculated.checksum(bytes[4..(8 + length as usize)].try_into()?);
